...
import { DO, DEVICE_TYPE_PLC, DEVICE_TYPE_DO8 } from '../../../constants';
...
const Di = c(({
  ...
  for (let i = 1; i <= n; i += 1) {
    const { title, code } = get(get(`${id}/${DO}/${i}`).bind);
    a.push((
  ...
  return (
    <SimpleMenu handle={<Button>{DO} {index}</Button>}>
      {a}
    </SimpleMenu>
  );
});
...
class Container extends Component<Props> {
  ...
  selectDo = (index) => {
    this.setState({ index });
    this.props.onSelect(`${this.state.dev}/${DO}/${index}`);
  }
  render() {
      ...
      <table>
        <tbody>
          <tr>
            ...
            <td className="paper">
              <Di id={dev} index={index} onSelect={this.selectDo} />
            </td>
          </tr>
        </tbody>
      </table>
